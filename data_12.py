...
def main():
    ...
    parser.add_argument('--alg', help='SMVR, scsc, ogd, npag, Tscgd, NLASG, NASA, easy', type=str, default='improve')
    ...
    make_log_file(alg, postfix='_12')

    path = "./data/12/12_Industry_Portfolios_Daily_1.csv"
    ...
    for i in range(trials):
        ...
        for j in range(iters):
            ...
            for ind in range(int(len(train_X) / batch_size)):
                ...
                if alg == 'ogd':
                    ...
                    theta, grad, layer1_before = NASA(theta, grad, layer1_before, train_X_batch, lamda, alpha, beta=0.1, M=M)
            ...
            if (j + 1) % interval == 0:
                ...
                print(f'[{j + 1} / {iters}] avg loss: {np.mean(np.array(fw_list))}')
                print(f'[{j + 1} / {iters}] avg gradient: {np.mean(np.array(gm_list))}')
        ...
        print(f'[trials: {i}] avg loss: {np.mean(np.array(fw_list))}')
        print(f'[trials: {i}] avg gradient: {np.mean(np.array(gm_list))}')
    ...
    if not os.path.exists('./figure_12/Loss'):
        os.makedirs('./figure_12/Loss')
    if not os.path.exists('./figure_12/Gradient'):
        os.makedirs('./figure_12/Gradient')
    if not os.path.exists('./figure_12/FW'):
        os.makedirs('./figure_12/FW')
    if not os.path.exists('./figure_12/GradientMapping'):
        os.makedirs('./figure_12/GradientMapping')
        
    to_csv(sample_list, loss_all_list, './figure_12/Loss/', alg, '_loss.csv')
    to_csv(sample_list, gradient_all_list, './figure_12/Gradient/', alg, '_gradient.csv')
    to_csv(sample_list, fw_all_list, './figure_12/FW/', alg, '_fw.csv')
    to_csv(sample_list, gm_all_list, './figure_12/GradientMapping/', alg, '_gm.csv')
