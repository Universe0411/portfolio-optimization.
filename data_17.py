...
def main(alg,lr=0.1,alpha=0.1,beta=0.5,M=50):
    parser = argparse.ArgumentParser()
    parser.add_argument('--trials', type=int, default=10)
    parser.add_argument('--iters', type=int, default=10)
    parser.add_argument('--lr', type=float, default=0.01)
    parser.add_argument('--eval_interval', type=int, default=1)
    parser.add_argument('--alg', help='SMVR, scsc, ogd, npag, Tscgd, NLASG, NASA, easy', type=str, default='easy')
    ...
    make_log_file(alg, postfix='_17')

    path = "./data/17/17_Industry_Portfolios_Daily_1.csv"
    ...
    write_log(batch_size, trials, iters, lr, interval, alg, lamda, alpha, avg_loss_list, avg_gradient_list,
              postfix='_17')
    if not os.path.exists('./figure_17/Loss'):
        os.makedirs('./figure_17/Loss')
    if not os.path.exists('./figure_17/Gradient'):
        os.makedirs('./figure_17/Gradient')
    if not os.path.exists('./figure_17/FW'):
        os.makedirs('./figure_17/FW')
    if not os.path.exists('./figure_17/GradientMapping'):
        os.makedirs('./figure_17/GradientMapping')

    to_csv(sample_list, loss_all_list, './figure_17/Loss/', alg, '_loss.csv')
    to_csv(sample_list, gradient_all_list, './figure_17/Gradient/', alg, '_gradient.csv')
    to_csv(sample_list, fw_all_list, './figure_17/FW/', alg, '_fw.csv')
    to_csv(sample_list, gm_all_list, './figure_17/GradientMapping/', alg, '_gm.csv')

    return np.mean(np.array(avg_loss_list))
